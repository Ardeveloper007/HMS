...
namespace HMS.Sale
{
    public partial class frmSaleReturn : Form
    {
        ...
        int PaymentId = 0;
        public frmSaleReturn( UserAccount getuser)
        {
            InitializeComponent();
            user = getuser;
        }
        ...
        public void orderNoBind()
        {
            try
            {
                var cat = db.tbl_SaleMaster.ToList();
                DataTable dtcat = new DataTable();
                dtcat.Columns.Add("Id");
               ...
               int SPGLId = 0;
                ...
                if (chkPostVoucher.Checked == true)
                {
                    ...
                    else
                    {
                        ...
                        GetCode();
                tbl_SaleReturnMaster master = new tbl_SaleReturnMaster();
               master.OrderNo=txtPO.Text.ToString();
                master.ManualBillNo = txtBillNo.Text;
                master.DocumentTypeId = 4;
                ...
                master.tbl_SaleReturnDetail = new List<tbl_SaleReturnDetail>();
                foreach (Janus.Windows.GridEX.GridEXRow r in grddetail.GetRows())
                {
                    tbl_SaleReturnDetail detail = new tbl_SaleReturnDetail();
                    detail.SaleReturnMaster_Id = master.Id;
                    ...
                    master.tbl_SaleReturnDetail.Add(detail);
                }
                db.tbl_SaleReturnMaster.Add(master);
                db.SaveChanges();
                int scId = Numerics.GetInt(ddlSupplierCustomer.Value);
               
                var getMaxId = db.tbl_SaleReturnMaster.OrderByDescending(x => x.Id).ToList();
                if (getMaxId.Count > 0)
                {
                    if (getMaxId[0].Id > checkid)
                    {
                        ...
                        foreach (Janus.Windows.GridEX.GridEXRow r in grddetail.GetRows())
                        {
                            ...
                            vd.Comments = "supplier Debit for saleReturn";
                            ...
                            vd1.COA_Id = SPGLId;
                            vd1.Comments = "Item Credit for SaleReturn";
                        ...
                        if(chkPostVoucher.Checked == true)
                        {
                            ...
                            if (PaymentId > 0)
                            {
                                ...
                                vd2.COA_Id = SPGLId;
                                vd2.Comments = "Amount Added To Supplier A/C from Sale Return";
                                ...
                                vd3.Comments = "Amount deducted from Cash in Hand from Sale Return";
                                vd3.CreditAmount = 0;
                        ...
                        db.SaveChanges();
                            ...
                            foreach (Janus.Windows.GridEX.GridEXRow r in grddetail.GetRows())
                            {
                                ...
                                transaction.RefDocumentTypeId = 4;
                                ...
                                transaction.QtyIn = Numerics.GetInt(r.Cells["Qty"].Value);
        ...
        private void GetCode()
        {
            ...
            {
                int PONo = 0;
                var PO = db.tbl_SaleReturnMaster.OrderByDescending(x => x.Id).ToList();
                if (PO.Count > 0)
                {
                    string purchaseNumber = PO[0].OrderNo.ToString();
                    int count = int.Parse(purchaseNumber.Trim(new Char[] { 'S', 'R', '-' })) + 1;
                    txtPO.Text = "SR-" + count;
                }
                else
                {
        ...
        private void Calculations()
        {
            try
            {
               ...
               double TotalAmount = Qty * AmountPerPack;
                ...
                double unitPrice = Numerics.GetDouble(txtunitPrice.Text);
                txtunitPrice.Text = unitPrice.ToString();
                txtTotalAmount.Text = (Qty * unitPrice).ToString();
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
        ...
        private void grdSettings()
        {
            ...
            grddetail.RootTable.Columns["Qty"].EditType = EditType.NoEdit;
            grddetail.RootTable.Columns["Rate"].EditType = EditType.NoEdit;
            grddetail.RootTable.Columns["SalePrice"].EditType = EditType.NoEdit;
            grddetail.RootTable.Columns["Amount"].EditType = EditType.NoEdit;
            grddetail.RootTable.Columns["ExpiryDate"].EditType = EditType.NoEdit;
        ...
        private void ddlOrderNo_Leave(object sender, EventArgs e)
        {
            try
            {
                ...
                if (ddlOrderNo.Text != string.Empty)
                {
                    ...
                    var PODetail = (from h in db.tbl_SaleMaster
                                    join
    d in db.tbl_SaleDetail on h.Id equals d.SaleMaster_Id
                    ...
                    if (PODetail.Count > 0)
                    {
                        ...
                        for (int i = 0; i < PODetail.Count; i++)
                        {
                           ...
                           PODetail[i].UnitRate.ToString(),
                           PODetail[i].BillAmount.ToString(),
        ...
        private void grddetail_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                ...
                if (item != null)
                {
                    ...
                    txtunitPrice.Text = item.Cells["SalePrice"].Value.ToString();
        ...
        private void btnUpdateGridRecord_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Numerics.GetDouble(txtQty.Text) > Numerics.GetDouble(dtGrid.Rows[updateDetailIndex]["Qty"]))
                {
                    MessageBox.Show("Qty Is greate than sale Qty");
                    txtQty.Focus();
                    return;
                }
                ...
                dtGrid.Rows[updateDetailIndex]["SalePrice"] = txtunitPrice.Text;
        ...
        public void History()
        {
            try
            {
                var history = (from h in db.tbl_SaleReturnMaster
        ...
        private void grdHistory_ColumnButtonClick(object sender, ColumnActionEventArgs e)
        {
            try
            {
                ...
                var dbdetail = (from d in db.tbl_SaleReturnDetail
                                join
c in db.tblCategories on d.CategoryId equals c.Id
                                ...
                                where d.SaleReturnMaster_Id == rId
                                select new
                                {
                                    d.ItemQty,
                                    d.ItemRate,
                                    d.ItemAmount,
                                    d.RemarksDetail,d.UnitRate,
                                    i.Item_Name,
                                    c.Category_Name
                                }).ToList();
                ...
                {
                    dtdetailhistory.Rows.Add(dbdetail[i].Category_Name, dbdetail[i].Item_Name, dbdetail[i].ItemQty, dbdetail[i].UnitRate, dbdetail[i].ItemAmount);
                }
