...
namespace HMS.Purchase
{
    public partial class frmPurchase : Form
    {
        ...
        int PaymentId = 0;
        public frmPurchase( UserAccount getuser)
        {
            InitializeComponent();
            user = getuser;
        }
        ...
        private void frmPurchase_Load(object sender, EventArgs e)
        {
            ...
            dtGrid.Columns.Add("PackQty");
            dtGrid.Columns.Add("Qty",typeof(double));
            dtGrid.Columns.Add("NetQty",typeof(double));
            dtGrid.Columns.Add("Rate");
            dtGrid.Columns.Add("UnitRate");
        ...
        private bool formvalidation()
        {
            ...
            if (ddlSupplierCustomer.Text == string.Empty)
            {

                MessageBox.Show("SupplierCustomer Field Requird!!!");
            ...
            {
                MessageBox.Show("txtTotal Amount Field Requird!!!");
                txtTotal.Focus();
            ...
            {
                MessageBox.Show("Paid Field Requird!!!");
                txtPaid.Focus();

                return false;
            }
            if (txtRemaining.Text.Trim() == "")
            {
                MessageBox.Show("Remaining No Field Requird!!!");
                txtRemaining.Focus();
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                if (ValidateDetails() && ChkGrid())
                {
                    ...
                    if (txtexpirydate.Value <= onemonth)
                    {
                        MessageBox.Show("Enter Correct Expiry Date correct.... ");
                        txtexpirydate.Focus();
                        return;
                    }
                    dtGrid.Rows.Add
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                master.tbl_PurchaseDetail = new List<tbl_PurchaseDetail>();
                foreach (Janus.Windows.GridEX.GridEXRow r in grddetail.GetRows())
                {
                    tbl_PurchaseDetail detail = new tbl_PurchaseDetail();
                    detail.PurchaseMaster_Id = master.Id;
                    ...
                    detail.NetQty = Numerics.GetDouble(r.Cells["NetQty"].Value);
                    detail.ItemRate = Numerics.GetDouble(r.Cells["Rate"].Value);
                    detail.UnitRate = Numerics.GetDouble(r.Cells["UnitRate"].Value);
                    detail.ItemAmount = Numerics.GetDouble(r.Cells["Amount"].Value);
                    detail.BillAmount = Numerics.GetDouble(txtTotal.Text);
                    detail.ExpiryDate = Convert.ToDateTime(txtexpirydate.Value);
                    detail.RemarksDetail = "";
                    master.tbl_PurchaseDetail.Add(detail);
                }
                db.tbl_PurchaseMaster.Add(master);
                db.SaveChanges();
                ...
                if (getMaxId.Count > 0)
                {
                    if (getMaxId[0].Id > checkid)
                    {
                        ...
                        foreach (Janus.Windows.GridEX.GridEXRow r in grddetail.GetRows())
                        {
                            ...
                            vd.Comments = "Product Debit for Purchase";
                            vd.DebitAmount = Numerics.GetDouble(r.Cells["Amount"].Value);
                            vd.CreditAmount = 0;
                            ...
                            vd1.COA_Id = SPGLId;
                            vd1.Comments = "Supplier Credit for Purchase";
                            vd1.CreditAmount = Numerics.GetDouble(r.Cells["Amount"].Value);
                            vd1.DebitAmount = 0;
                        ...
                        if(chkPostVoucher.Checked == true)
                        {
                            ...
                            if (PaymentId > 0)
                            {
                                ...
                                vd2.COA_Id = SPGLId;
                                vd2.Comments = "Amount Added To Supplier A/C from Purchase";
                                vd2.CreditAmount = Numerics.GetDouble(txtPaid.Text);
                                vd2.DebitAmount = 0;
                                ...
                                vd3.Comments = "Amount deducted from Cash in Hand from Purchase";
                                vd3.CreditAmount = 0;
                                vd3.DebitAmount = Numerics.GetDouble(txtPaid.Text);
                                // vd2.Item_Id = Numerics.GetInt(r.Cells["ProId"].Value);
                                vd3.ChequeNo = "";
                        ...
                        foreach (Janus.Windows.GridEX.GridEXRow r in grddetail.GetRows())
                        {
                            ...
                            transaction.RefDocumentTypeId = 1;
                            ...
                            transaction.QtyIn = Numerics.GetDouble(r.Cells["NetQty"].Value);
        ...
        private void GetCode()
        {
            ...
            {
                int PONo = 0;
                var PO = db.tbl_PurchaseMaster.OrderByDescending(x => x.Id).ToList();
                if (PO.Count > 0)
                {
                    string purchaseNumber = PO[0].OrderNo.ToString();
                    int count = int.Parse(purchaseNumber.Trim(new Char[] { 'P', 'O', '-' })) + 1;
                    txtPO.Text = "PO-" + count;
                }
                else
                {
        ...
        public void History()
        {
            try
            {
                var history = (from h in db.tbl_PurchaseMaster
        ...
        private void grdSettings()
        {
            ...
            grddetail.RootTable.Columns["PackQty"].EditType = EditType.TextBox;
            grddetail.RootTable.Columns["Rate"].EditType = EditType.TextBox;
            grddetail.RootTable.Columns["Amount"].EditType = EditType.TextBox;
            grddetail.RootTable.Columns["NetQty"].EditType = EditType.TextBox;
            ...
            grddetail.RootTable.Columns["PackQty"].FilterEditType = FilterEditType.TextBox;
            ...
            grddetail.RootTable.Columns["NetQty"].FilterEditType = FilterEditType.TextBox;
            ...
            grddetail.RootTable.Columns["NetQty"].AggregateFunction = Janus.Windows.GridEX.AggregateFunction.Sum;
            ...
            grddetail.RootTable.Columns["Qty"].Width = 70;
        ...
        private void grdHistory_ColumnButtonClick(object sender, ColumnActionEventArgs e)
        {
            try
            {
                ...
                var dbdetail = (from d in db.tbl_PurchaseDetail
                                join
c in db.tblCategories on d.CategoryId equals c.Id
                                ...
                                select new
                                {
                                    ...
                                    d.ItemAmount,
                                    d.RemarksDetail,
                                    i.Item_Name,c.Category_Name,d.ExpiryDate
                                }).ToList();
                ...
                dtdetailhistory.Columns.Add("ExpiryDate");
                for(int i=0;i<dbdetail.Count;i++)
                {
                    dtdetailhistory.Rows.Add(dbdetail[i].Category_Name, dbdetail[i].Item_Name, dbdetail[i].ItemQty, dbdetail[i].ItemRate, dbdetail[i].ItemAmount, Convert.ToDateTime(dbdetail[i].ExpiryDate).ToString("dd-MMM-yyyy"));
                }
        ...
        public void HistorydetailSetting()
        {
            try
            {
                ...
                grdHistoryDetail.RootTable.Columns["ExpiryDate"].EditType = EditType.NoEdit;
                ...
                grdHistoryDetail.RootTable.Columns["ExpiryDate"].FilterEditType = FilterEditType.TextBox;

                grdHistoryDetail.RootTable.Columns["Category"].Width = 200;
                grdHistoryDetail.RootTable.Columns["Product"].Width = 400;
                ...
                grdHistoryDetail.RootTable.Columns["ExpiryDate"].Width = 100;
